  ...
  useEffect(() => {
    ...
    getAllDocs("navItems").then((snap) => {
      const items = snap.docs.map(
        (doc) => ({ ...doc.data(), id: doc.id } as INavItem)
      );
        ...
        if (!item.isPage) {
          ...
          if (item.features) {
            item.features.forEach((feature) => {
              if (
                lvl1Items.some((existItem) =>
                  existItem.includes(item.name + " > " + feature.name)
                )
              ) {
                return;
              }
          ...
          if (item.navItems) {
            item.navItems.forEach((lvl1) => {
              if (
                lvl1Items.some((existItem) =>
                  existItem.includes(item.name + " > " + lvl1.name)
                )
              ) {
                return;
              }
              ...
              if (lvl1.navItems) {
                lvl1.navItems.forEach((lvl2) => {
                  if (
                    lvl2Items.some((existItem) =>
                      existItem.includes(
                        item.name + " > " + lvl1.name + " > " + lvl2.name
                      )
                    )
                  ) {
                    return;
            ...
            <div className="mt-1">
              ...
              <Editor
                defaultContentState={convertToRaw(
                  convertFromRaw(product.description)
                )}
          ...
          <div className="sm:col-span-2">
            <Combobox
              ...
              onSelect={(value: string) => {
                ...
                setFilteredLvl1Nav(
                  lvl1Nav.filter((item) => item.includes(value))
                );
              }}
            />
          </div>
          <div className="sm:col-span-2">
            <Combobox
              ...
              onSelect={(value: string) => {
                ...
                setfilteredLvl2Nav(
                  lvl2Nav.filter((item) => item.includes(value))
                );
              }}
            />
          </div>
